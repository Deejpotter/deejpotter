...
describe("ContactForm", () => {
  ...
  test("server error shows error message", async () => {
    global.fetch = vi.fn(() =>
      Promise.resolve({ ok: false, json: () => ({ message: "Bad" }) } as any)
    ) as any;
    ...
    await waitFor(() =>
      expect(
        screen.getByText(
          /Failed to submit the form|An error occurred while submitting the form|Bad/i
        )
      ).toBeInTheDocument()
    );

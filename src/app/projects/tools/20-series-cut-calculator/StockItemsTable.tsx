...
// StockItemsTable (Tailwind-first)
...
export default function StockItemsTable({
  ...
  // Simplified helpers for now. Full table restored later in a follow-up step.
  const handleAddStockItem = (): void => {
    onStockItemsChange([
      ...stockItems,
      { id: String(Math.max(0, ...stockItems.map((s) => parseInt(s.id) || 0)) + 1), length: maxStockLength, quantity: 10 },
    ]);
  };
    ...
    <div className="rounded shadow-sm bg-white dark:bg-gray-800 text-black dark:text-white">
      ...
      <div className="p-0">
            ...
            <tbody>
              {stockItems.length === 0 ? (
                ...
                stockItems.map((stock) => (
                  <tr key={stock.id}>
                    <td className="align-middle"><strong>#{stock.id}</strong></td>
        ...
        <div className="px-4 py-2 text-sm text-gray-600 dark:text-gray-300">
          <small>
            Total stock pieces: {stockItems.reduce((sum, s) => sum + s.quantity, 0)} | Total stock length available: {totalStockLength.toLocaleString()}mm
          </small>
        </div>
      </div>
    </div>
